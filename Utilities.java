   //Mash, Mangle & Munch - Rev. Dr. Douglas R Oberle, June 2012  doberle@fcps.edu
  ...
  //handles miscelaneous methods and file IO for highscores
    public class Utilities extends MMMPanel
   {
       ...
       public static double distance(int x1, int y1, int x2, int y2)
      {
   	...
   	//pre:  x1 - x2 != 0
      //post: returns true if the point (x,y) is within (cellSize/2) units from the segment defined by (x1,y1) & (x2, y2)
       public static boolean isPointOnSegment(int x, int y, int x1, int y1, int x2, int y2)
      {
       ...
       public static void readFile(String fileName,  playerScore [] scores)
      {
         try
         {
            ...
            while (input.hasNextLine() && i < scores.length)		//while there is another line in the file
            {
               try
               {
                  damage=input.nextLong();			//read in the next propertyDamage in the file and store it in line
                  name = input.next();			//monster name
                  time = input.nextInt();			//time expired
               }
   ...
   //post:  automatically stomps on any structrure with a small enough health when moved over it
       public static void autoStomp(Monster curr, int r, int c, int p)
      {
         if(pause || curr==null || curr.getName().equals("NONE") || curr.isFlying())
            return;
         int strHealth = curr.autoStompPower();
