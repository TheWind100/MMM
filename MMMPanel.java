   ...
   {	
    ...
    //we need an array of these ArrayLists to store locations across multiple panels (of which there are 9)
       ...
       public MMMPanel()
      {
         ...
         playerImages = new String[7][4][4][4]; 			//[monster #]x[body direction]x[animation frames]x[head positions]
         //structures
     ...
     //post:  checks for a physical collision between monster curr and vehicle unit at index i
            ...
            for(int j=0; j<bullets.size(); j++)			//see if bullets hit any enemies
            {
               ...
               {		//if bullet is an air shot and target is not an airplane nor one of the monsters
                  ...
                  if(shot.getOwner()==i)				//don't shoot ourselves
                     continue;
                  if(!name.startsWith("CROWD") && (curr instanceof Vehicle) && shot.getType().equals("BULLET"))
                     continue;							//only CROWDS are effected by bullets
                  if(curr instanceof Vehicle)		//***VEHICLE GETS HIT
                  {	
                     if(gameMode!=EARTH_INVADERS && shot.getType().startsWith("SHRIEK") && !curr.isFlying() && !name.startsWith("TRAIN"))
                     {
                        if(shot.getType().startsWith("SHRIEK"))
       ...
       public static void playerShoot(Player current, int owner)
      {
         ...
         {	//everyone can shoot in EARTH_INVADERS, so reload if time - don't shoot and show message
            ...
            if(curr.isThrower())
            {	//Gorilla has to have ammo in hand, otherwise, don't shoot and show message
               if(curr.projectileType().equals("empty"))
                  needToReload = true;
            }
            else
            {	//non-Gorillas in non-EARTH_INVADERS mode might have reload time
               if((numFrames > curr.getReloadTime() && numFrames < curr.getLastShotTime() + curr.getReloadTime()))
                  needToReload = true;
            }
         if(needToReload)          
         ...
         if(gameMode==EARTH_INVADERS || (curr.isThrower() && !curr.projectileType().equals("empty"))|| (curr.isShooter() && curr.getHunger() < hungerLevels.length -1) || curr.energyAbsorber())
         {//always allowed to shoot in earth invaders mode, otherwise, shooter monster types only shoot if they are not weakened from hunger
          //thrower monster types can only shoot if holding a vehicle - Robot can always shoot
    ...
    //post:directs monster or vehicle depending on keyboard input k
       public static void playerMove(int k, int playerIndex)
      {
         ...
         {	//stomp command for p1 || p2
            if(curr.isFlyer() && gameMode!=EARTH_INVADERS)	//insect wants to fly, so see if it has enough stamina to takeoff and land
            {
               if((numFrames > curr.getReloadTime()/2 && numFrames < curr.getLastShotTime() + curr.getReloadTime()/2))
            ...
            {	//up command for player 1 || player 2 
               if(curr.getBodyDirection() == UP && r>0 && MapBuilder.noStructure(r-1, c, panel) || curr.isFlying() )
               {
                  if(r==1)		//top row	
                  {
               ...
               {	//down command for player 1 || player 2 
                  if(curr.getBodyDirection() == DOWN && r < board.length-1 && MapBuilder.noStructure(r, c, panel) || curr.isFlying() )
                  {
       ...
       public static void movePlayerSmoothly()
      {
         for(int i=0; i < players.length; i++)
         {
            ...
            {
               Utilities.autoStomp((Monster)(curr), r, c, panel);
            ...
            {	//put out any fire close to the firetruck
               for(int nr=r-1; nr<=r+1; nr++)
                  for(int nc=c-1; nc<=c+1; nc++)
                  {
                     if(nr<1 || nc<1 || nr>board.length-1 || nc>board[0].length-1)
                        continue;
                     if(structures[nr][nc][panel]!=null && structures[nr][nc][panel].onFire())
       ...
       public void mouseClicked( MouseEvent e )
      {
         ...
         if (monsterMaker)	
         {
            if(highlightArea >= 0 && highlightArea<24)
