    ...
    public abstract class Monster extends Player
   {
      ...
      private int autoStompPower;		//damage done by just walking on a structure (but not jumping on it)
      private int hunger;
     ...
     //ARGS:  name, row, col, image collection, animation delay, stomp power, speed penalty, reload time, autoStompPower, projectileType, burnDamage 
       public Monster(String n, int r, int c, String[][][] image, int ad, int sp, int spp, int rt, int asp, String pt, int bd)
      {
         super(n, r, c, image, ad, spp, rt);
      //ARGS:  name, row loc, col loc, image collection, animation delay, stomp power, speed penalty, reload time			
   ...
   //returns the damage that a monster does to a structure just by walking on it (not stomping) - each monster can deal a different amount of damage
       public int autoStompPower()
      {
         return autoStompPower;
      }
