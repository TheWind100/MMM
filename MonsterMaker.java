//Mash, Mangle & Munch - Rev Dr Douglas R Oberle, July 2012  doberle@fcps.edu
    ...
    public class MonsterMaker extends MMMPanel
   {
     ...
     //pre:   g != null
       ...
       public static void monsterLab(Graphics g)
      {
         ...
         setTextColor(g, 8);
         g.drawString("SOFT HIDE       :"+customInfo[21], x, y+=size);
         setTextColor(g, 9);
         g.drawString("LOWER APPETITE  :"+customInfo[19], x, y+=size);
         setTextColor(g, 10);
         g.drawString("BULLET DEFLECTOR:"+customInfo[20], x, y+=size);
         setTextColor(g, 11);
         g.drawString("TURN HEAD 360   :"+customInfo[9], x, y+=size);
         setTextColor(g, 12);
         g.drawString("SWIMMER         :"+customInfo[10], x, y+=size);
         setTextColor(g, 13);
         g.drawString("HEAL IN WATER   :"+customInfo[16], x, y+=size);
         setTextColor(g, 14);
         g.drawString("DROWN IN WATER  :"+customInfo[18], x, y+=size);
         setTextColor(g, 15);
         g.drawString("FLYER           :"+customInfo[11], x, y+=size);
         setTextColor(g, 16);
         g.drawString("DIGGER          :"+customInfo[12], x, y+=size);
         setTextColor(g, 17);
         g.drawString("THROWER         :"+customInfo[13], x, y+=size);
         setTextColor(g, 18);
         g.drawString("AMBIDEXTOROUS   :"+customInfo[17], x, y+=size);
         setTextColor(g, 19);
         g.drawString("SHOOTER         :"+customInfo[14], x, y+=size);
         setTextColor(g, 20);
         g.drawString("SPLITTER        :"+customInfo[15], x, y+=size);
         setTextColor(g, 21);
         g.drawString("TOXIC TRAIL     :"+customInfo[22], x, y+=size);
         setTextColor(g, 22);
         g.drawString("ENERGY ABSORBER :"+customInfo[23], x, y+=size);
         setTextColor(g, 23);
         g.drawString("CAN GRAB/EAT ALL:"+customInfo[24], x, y+=size);
       ...
       protected static void changeAttribute(int button)
      {
         ...
         {//change monster body type
            ...
            if(newType != 2 && Boolean.parseBoolean(customInfo[9])==true)    			 
               customInfo[9] = "false";	//only the Robot gets the 360 degree head turn
         ...
         {//360 head
            ...
            if(bodyType == 2  || bodyType == 5 || bodyType == 6)		//only the robot, worm and ufo gets 360 head turn option
            {
     ...
     //post:displays monster attribute narratives depending on which feature is highlighted (as opposed to showing key commands)
       private static int showAttributes(Graphics g, int x, int y, int size)
      {
         ...
         {
            g.drawString("Monster body type (1-7)   ", x, y+=((int)(size*.75)));
         ...
         else if(highlightArea==11)
         {
            ...
            g.drawString("Worm and UFO.             ", x, y+=((int)(size*.75)));
            y+=((int)(size*.75)*7);
            cost = "-100";
         }
         else if(highlightArea==12)
         ...
         else if(highlightArea==19)
         {
            ...
            g.drawString("not be a thrower nor a    ", x, y+=((int)(size*.75)));
            g.drawString("splitter.                 ", x, y+=((int)(size*.75)));
            y+=((int)(size*.75)*7);
            cost = "-350 to 550";
         }
         else if(highlightArea==20)
         {
            ...
            g.drawString("shooter, a thrower or a   ", x, y+=((int)(size*.75)));
            g.drawString("splitter.                 ", x, y+=((int)(size*.75)));
            y+=((int)(size*.75));
            cost = "-500";
         }
